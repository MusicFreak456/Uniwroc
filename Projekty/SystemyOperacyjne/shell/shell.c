...
static int do_job(token_t *token, int ntokens, bool bg) {
  ...
  setpgid(child_pgid, child_pgid);

  MaybeClose(&input);
  MaybeClose(&output);

  int job_id = addjob(child_pgid, bg);
  addproc(job_id, child_pid, token);
...
static int do_pipeline(token_t *token, int ntokens, bool bg) {
  ...
  while (start < token + ntokens) {
    int stage_ntokens = 0;
    while (start[stage_ntokens] != T_PIPE && start[stage_ntokens] != T_NULL) {
      stage_ntokens++;
    }
    start[stage_ntokens] = NULL;
    token_t *next = start + stage_ntokens + 1;

    if (next < token + ntokens) {
      pid = do_stage(pgid, &mask, input, output, start, stage_ntokens, bg);
      MaybeClose(&input);
      MaybeClose(&output);
      input = next_input;
      mkpipe(&next_input, &output);
    } else {
      pid = do_stage(pgid, &mask, input, -1, start, stage_ntokens, bg);
      MaybeClose(&input);
    }
