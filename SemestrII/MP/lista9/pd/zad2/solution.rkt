...
;Zadanie rozwiazanie wspólnie z Krystianem Jasionkiem i Bartoszem Janikowskim
...
(define (op->proc op)
  (match op ['+ +] ['- -] ['* *] ['/ /] ['% modulo]
            ...
            ['eq? eq?]
            [_ false]))
;-----------------ZMIENIONY EVAL-ENV TERAZ DWA ŚRODOWISKA---------------------
(define (eval-env e env env-while)
  (match e
    ...
    [(var-expr x) (if (env-member? x env)
                      (env-lookup x env)
...
(define (test1)
  (let* ([init-env  env-empty]
         [final-env
           (eval-while (parse-while TEST1) init-env)])
...
(define (test2)
  (let* ([init-env  env-empty]
         [final-env
           (eval-while (parse-while TEST2) init-env)])
    (env-lookup 'z final-env)))
(test2)
...
(define (test3)
  (let* ([init-env  env-empty]
         [final-env
           (eval-while (parse-while TEST3) init-env)])
    (env-lookup 'z final-env)))
(test3)
...
(define (test4)
  (let* ([init-env  env-empty]
         [final-env
           (eval-while (parse-while TEST4) init-env)])
    (env-lookup 'z final-env)))
(test4)
