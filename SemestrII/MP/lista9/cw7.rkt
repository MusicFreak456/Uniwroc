...
(struct print-expr  (e)        #:transparent)
...
(define (parse q)
  (cond
    ...
    [(and (list? q) (eq? (length q) 1) (eq? (first q) 'read))
     (read-expr)]
    [(and (list? q) (eq? (length q) 2) (eq? (first q) 'print))
     (print-expr (parse (second q)))]
    [(and (list? q) (> (length q) 0) (eq? (first q) 'begin))
     (begin-expr (parse (second q)))]
...
(define (eval-env e env)
  (match e
    [(const n) n]
    [(read-expr) (eval-env (parse (read)) env)]
    [(print-expr e) (begin
                      (print (eval-env e env))
                      (void))]
    [(begin-expr es) (void)]
