  ...
  (cond
    ...
    [(and (list? q) (= (length q) 4) (eq? (car q) 'if))
     ...
     [(and (list? q) (= (length q) 5) (eq? (car q) 'for))
      (let ([init-instr (parse-while (second q))]
            [condition (parse (third q))]
            [post-instr (parse-while (fourth q))]
            [main-instr (parse-while (fifth q))])
        (comp init-instr
              (while condition (comp main-instr post-instr))))]
     ...
     [(and (list? q) (> (length q) 2))
      (comp (parse-while (first q))
            (parse-while (cdr q)))]
     [else (error "while parse error" q)]))
...
(define FOR_TEST
  '{
    (b := 0)
    (for (i := 0) (< i 2) (i := (+ i 1)) (b := (+ b i)))
    })

(parse-while FOR_TEST)
(define (fortest)
  (let* ([init-env  env-empty]
         [final-env
           (eval-while (parse-while FOR_TEST) init-env)])
    (env-lookup 'b final-env)))
(fortest)
