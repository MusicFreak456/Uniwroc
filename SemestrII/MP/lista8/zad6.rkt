...
(define unops (set 'car 'cdr 'not))

(define (desugar-magic symbol expr)
  (define (iter-desugar list-sym)
    (match (first list-sym)
      [#\c (iter-desugar (cdr list-sym))]
      [#\a (unop 'car (iter-desugar (cdr list-sym)))]
      [#\d (unop 'cdr (iter-desugar (cdr list-sym)))]
      [#\r expr]))
  (iter-desugar (string->list (symbol->string symbol)))) 
  ...
  (cond
    ...
    [(and (list? q) (eq? (length q) 3) (eq? (first q) 'cons)) ; <- !!!
     (cons-expr (parse (second q))
                (parse (third q)))]
    [(and (list? q) (eq? (length q) 2) (set-member? unops (first q))) ; <-- !!!
    ...
    [(and (list? q) (eq? (length q) 2) (symbol? (first q)))
     (desugar-magic (first q) (parse (second q)))]
))
...
(define (unop->proc op)
  (match op ['car car] ['cdr cdr] ['not not]))
...
(define (eval-env e env)
  (match e
    ...
    [(binop op l r) ((op->proc op) (eval-env l env)
                                   (eval-env r env))]
    [(unop op arg) ((unop->proc op) (eval-env arg env))]
    [(let-expr x e1 e2)
...
(parse '(caadr (cons 1 2)))

(parse '(caadaadr (cons 1 2)))
