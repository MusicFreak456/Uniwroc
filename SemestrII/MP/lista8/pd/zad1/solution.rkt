...
(define (parse q)
  (cond
    ...
    [(and (list? q) (eq? (first q) 'list))
     (foldr cons-expr (null-expr) (map parse (cdr q)))]
    ;;Tutaj druga istotna zmiana--------------------------------
    [(and (list? q) (eq? (first q) 'apply))
     (apply-expr (parse (second q)) (parse (third q)))]
    ;-----------------------------------------------------------
    ...
    [(apply-expr f xs)
     (let ([vf (eval-env f env)]
       ...
       (match vf
         [(clo x body fun-env)
          (cond
            [(null? (cdr vl)) (eval-env body (env-add x (car vl) fun-env))]
            [else (eval-env (apply-expr body (cdr-expr xs)) (env-add x (car vl) fun-env))])]
         [_ "ERROR"]))]
    ;;--------------------------------------------------------------
...
(displayln "1sze z zadania")
(eval (parse
       '(apply (lambda (x y) (+ x y))
                (list 1 2))))
(displayln "2gie z zadania")
(eval (parse
       '(apply (lambda (x y z) (+ x (+ y z)))
                (list 1 2))))
(displayln "3cie z zadania")
(eval (parse
       '(apply (lambda (x y) (lambda (z) (+ x (+ y z))))
                (list 1 2 3))))

(displayln "4te z zadania")
(eval (parse
       '(apply (lambda (x y) (+ x y))
                (list 1 2 3))))
