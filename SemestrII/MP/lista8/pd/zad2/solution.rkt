...
(define (eval-env e env)
  (match e
    [(const n) n]
    ;Tu dodatek--------------------------------------------
    [(thunk e thunk-env) (eval-env e thunk-env)]
    ;------------------------------------------------------
    ...
    [(var-expr x) (match (env-lookup x env)
                    [(thunk e1 thunk-env) (eval-env e1 thunk-env)]
                    [_ (env-lookup x env)])]
    ;------------------------------------------------------
    ...
    [(cons-expr e1 e2) (cons (thunk e1 env)
                             (thunk e2 env))]
    [(car-expr e) (eval-env (car (eval-env e env)) env)]
    [(cdr-expr e) (eval-env (cdr (eval-env e env)) env)]
    ;------------------------------------------------------
    [(null-expr) null]
...
;;Testy z zadania
(displayln "1szy test z zadania")
(eval (parse '((lambda (x) (+ 3 3)) (/ 5 0))))

(displayln "2gi test z zadania")
(eval (parse '(let [if-fun (lambda (b t e) (if b t e))]
          (if-fun true 4 (/ 5 0)))))

(displayln "3ci test z zadania")
