...
(struct unop     (op sub)   #:transparent)
...
(define (expr? e)
  (match e
    [(const n) (or (number? n) (eq? n 'true) (eq? n 'false))] ; <----------------- !!!
    [(unop op sub) (and (symbol? op) (expr? sub))]
...
(define (op->proc op)
  (match op ['+ +] ['- -] ['* *] ['/ /] ['% modulo] ; <----------- !!!
            ['= =] ['> >] ['>= >=] ['< <] ['<= <=]
            ['boolean? (lambda (sub) (or (= sub 1) (= sub 0)))]
            ['number? number?]))
...
(define (eval-env e env)
  (match e
    ...
    [(const 'false) 0]
    [(const n) n]
    [(binop 'and l r) (and (eval-env l env) (eval-env r env))]
    [(binop 'or l r) (or (eval-env l env) (eval-env r env))]
    [(binop op l r) ((op->proc op) (eval-env l env)
                                   (eval-env r env))]
    [(unop op sub) ((op->proc op) (eval-env sub env))]
    [(let-expr x e1 e2)
