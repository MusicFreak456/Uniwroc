...
(struct boolean?-expr (exp) #:transparent)
(struct number?-expr  (exp) #:transparent)
...
(define (expr? e)
  (match e
    ...
    [(boolean?-expr exp) (expr? exp)]
    [(number?-expr exp) (expr? exp)]
    [_ false]))
  ...
  (cond
    ...
    [(and (list? q) (eq? (length q) 3) (symbol? (first q)))
     (binop (first q)
            (parse (second q))
            (parse (third q)))]
    [(and (list? q) (eq? (length q) 2) (eq? (first q) 'boolean?)
          (boolean?-expr (parse (second q))))]
    [(and (list? q) (eq? (length q) 2) (eq? (first q) 'number?)
...
(define (eval-env e env)
  (match e
    ...
    [(if-expr eb et ef) (if (eval-env eb env) ; <----------------- !!!
                            (eval-env et env)
                            (eval-env ef env))]
    [(boolean?-expr exp) (boolean? (eval-env exp env))]
    [(number?-expr exp) (number? (eval-env exp env))]
))
