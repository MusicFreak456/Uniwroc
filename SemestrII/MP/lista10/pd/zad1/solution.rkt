...
(define effect-env
  (foldl (lambda (p env) (env-add (first p) (second p) env))
         start-env
         `((choose  ,(effect-builtin/2 (lambda (x y) (list (cons (/ 1 2) x) (cons (/ 1 2) y)))))
           (flip    ,(effect-builtin/3 (lambda (p x y) (list (cons p x) (cons (- 1 p) y)))))
           (uniform ,(effect-builtin/1 (lambda (xs) (let ([len (length xs)])
                                                      (foldr
                                                       (lambda (a b) (cons (cons (/ 1 len) a) b))
                                                       '()
                                                       xs)))))
           )))

;----------------------ZMODYFIKOWNY BIND I RETURN----------------------------------------------------

(define (bind c k) (append-map
                    (lambda (x)
                      (map
                       (lambda (y) (cons
                                    (* (car x) (car y))
...
(define (norm xs)
  (reverse (map (lambda (single-xs)
         (foldr (lambda (a b)
                  (cons
                   (+ (car a) (car b))
                   (cdr a)))
                (cons 0 (cdar single-xs))
                single-xs))
       (group-by (lambda (x) (cdr x)) xs))))
  ...
  '(letrec [from-to (lambda (x n)
                      (cons x
                            (if (= x n)
                                null
                                (from-to (+ 1 x) n))))]
     (let [throw-n-times (lambda (n)
                           (letrec [iter (lambda (s n)
                                           (let [dice (uniform (from-to 1 6))]
                                             (if (= s n)
                                                 dice
                                                 (+ dice (iter (+ s 1) n)))))]
                             (iter 1 n)))]
       (let [first-throw (throw-n-times 1)]
         (throw-n-times first-throw)))))
