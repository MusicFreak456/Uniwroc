...
(struct handle-expr (e1 e2 )        #:transparent)
...
(define (expr? e)
  (match e
    ...
    [(app-expr ef ea)    (and (expr? ef)
                              (expr? ea))]
    ((handle-expr e1 e2) (and (expr? e1) (expr? e2)))
    [_                   false]))
  ...
  (cond
    ...
    [(and (list? q)
     ...
     (letrec-expr (first (second q))
                  ...
                  (parse (third q)))]
    [(and (list? q)
          (= (length q) 3) (eq? (first q) 'handle))
     (handle-expr (parse (second q)) (parse (third q)))]
    [(and (list? q)
          (= (length q) 4)
          (eq? (first q) 'if))
...
(define (eval-env e env)
  (match e
    ...
    [(handle-expr e1 e2)
     (let ([c1 (eval-env e1 env)])
       (if (runtime-error? c1)
           (eval-env e2 env)
           c1))]

    [(let-expr x e1 e2)
     (let-error (eval-env e1 env) (lambda (v1)
