...
(define effect-env
  (foldl (lambda (p env) (env-add (first p) (second p) env))
         start-env
         `((put ,(effect-builtin/2 (lambda (id n) (lambda (s) (cons n (env-add id n s))))))
           (get ,(effect-builtin/1 (lambda (id) (lambda (s) (cons (env-lookup id s) s)))))
           )))

(define (bind state-modifier callback)
  (lambda (old-state)
...
(define (return value)
  (lambda (old-state) (cons value old-state)))
...
(define PROGRAM
  '(letrec [rand (lambda (x) (let [x (get false)]
                               (begin (put (% (* (+ x 17) 37) 397))
                                      x)))]
...
((eval (parse '(- 1 2))) env-empty)

((eval (parse '(let (ignore (lambda (x y) y))
                 (ignore (put "x" 10)
                         (ignore (put "y" 20)
                                 (+ (ignore (put "y" 30) (get "x"))
                                            (get "y")))))))
 env-empty)
